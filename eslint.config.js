...
export default [
  ...
  // Base neostandard config (replaces eslint-config-standard + plugins)
  ...neostandard({ ts: true, noStyle: true }),

  // TypeScript strict + stylistic type-checked configs scoped to JS files
  ...
  // JSDoc recommended-typescript-flavor for JS files (before custom rules)
  {
    files: ['**/*.{cjs,js,mjs}'],
    ...jsdocPlugin.configs['flat/recommended-typescript-flavor']
  },

  // Main config for all JS/CJS/MJS files
  {
    files: ['**/*.{cjs,js,mjs}'],
    languageOptions: {
      ecmaVersion: 'latest',
    ...
    rules: {
      ...
      // Allow deprecated API usage (tseslint.config is valid for v8)
